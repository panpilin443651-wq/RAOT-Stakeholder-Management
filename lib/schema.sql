-- สคีมา Postgres (Neon) ของระบบจัดการผู้มีส่วนได้ส่วนเสีย กยท.
-- เดิมเป็น SQLite แบบไฟล์ ย้ายมา Postgres เพราะ Vercel serverless เขียนดิสก์ไม่ได้
--
-- id ใช้ GENERATED BY DEFAULT AS IDENTITY เพื่อให้ seed ใส่ id ตรง ๆ ได้
-- และให้แอปปล่อยให้ฐานข้อมูลออกเลขเองได้ด้วย
-- (หลัง seed ต้องเลื่อน sequence ตามค่า id สูงสุด ดูท้ายไฟล์ lib/seed.ts)
...
-- ============ [100] ทะเบียนผู้มีส่วนได้ส่วนเสีย ============
...
CREATE TABLE IF NOT EXISTS plan_quarter_result (
  ...
  problem             TEXT,
  solution            TEXT,
  cumulative_percent  DOUBLE PRECISION,
